...
twitter_token <- create_token(app = "mabr", 
...
```{r}
stream_tweets(q = "election", timeout = 5, file_name = 'keywords2.json')
keyword_tweets <- tryCatch({parse_stream('test_jsons/keywords.json')},
                       error = function(e)
                           {print(paste("Retrying with alternative function after initial error when parsing file",
                                        'test_jsons/keywords.json'));
                           return(recover_stream('test_jsons/keywords.json'))})
head(keyword_tweets)
